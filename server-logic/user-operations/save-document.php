...
try {
    ...
    if ($draft_id > 0) {
        ...
        if (!empty($file_name) && !empty($file_path)) {
            // update file_name, file_path, file_type, file_size
        } else {
            // keep the old file info
            $file_name = $cur_file_name;
            $file_path = $cur_file_path;
            $file_type = $cur_file_type;
        ...
        if ($noChange && $existingActions == $submittedActions && $action === 'draft') {
            echo json_encode([
                'success' => false,
        ...
        // Only delete the old file if a new file was uploaded and file_path is not empty
        if ($cur_file_path && $cur_file_path !== $file_path && !empty($file_path) && file_exists($cur_file_path)) {
            @unlink($cur_file_path);
        }

        echo json_encode(['success' => true, 'message' => 'Draft saved successfully', 'draft_id' => $document_id]);
        exit;
    } else {
        ...
        $stmt = $conn->prepare("
            INSERT INTO tbl_documents 
                (user_id, subject, remarks, file_name, file_path, file_type, file_size, urgency, status, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', NOW())
        ");
        $stmt->bind_param(
            "isssssis",
